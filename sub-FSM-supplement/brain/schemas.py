...
class RobotDecision(BaseModel):
    ...
    The model output must include reasoning, user-facing reply text, and an
...
def parse_action(action_dict: dict) -> RobotAction:
    ...
    Args:
        action_dict: Dictionary containing a type field.

    Returns:
        The corresponding action model.
...
class OrderCheckDecision(BaseModel):
    ...
    - result=wrong + action(fix_order): user provided corrected order.
    - result=wrong + action=None: correction missing, needs follow-up question.
    """
