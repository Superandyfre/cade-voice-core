...
2. 对满足 `next_retry_ts <= now` 且 `attempt_count < outbox_max_attempts` 的条目执行重发
3. 重发后写 `published` 条目，递增 `attempt_count`、更新 `last_attempt_ts`、计算 `next_retry_ts`
4. `attempt_count >= outbox_max_attempts` 的条目标记为 `dead_letter`
5. `delivered` 条目不参与重试
...
1. `OrderInputClassifier`：12 类快速分类（cancel/menu/noise/out_of_scope 等）
